...
function showDedicationText() { //seguidores
  ...
  if (!text) {
    text = `se que las cosas no salieron muy bien y lo lamento estos dias he pensando mucho en nosotros yyyyy en lo que significas para mi JDHFJD no quiero presionarte solo queria preguntarte si estas dispuesta a ser mi san valentin otra vezz? no se si es otra vez jdfhdjf eso tamo.`;  } else {
...
function showSignature() {
  ...
  signature.textContent = firma ? decodeURIComponent(firma) : "Con amor vicentino<3";
...
function showCountdown() {
  ...
  let startDate = startParam ? new Date(startParam + 'T00:00:00') : new Date('2026-02-13F00:00:00'); 
  let eventDate = eventParam ? new Date(eventParam + 'T00:00:00') : new Date('2026-02-13F00:00:30');
  ...
  function update() {
    ...
    container.innerHTML =
      `quien sabe kasjda: <b>${days}</b> días<br>` +
      `nuestra siguente pelea: <b>${eventDays}d ${eventHours}h ${eventMinutes}m ${eventSeconds}s</b>`;
